...
/**
 * An object that provides information about the source of a data
 * provider related event. {@code DataProviderEvent} objects are
 * generated when a data provider experiences an operational error or a
 * state change resulting from configuration updates or administrative
 * actions.
 * <p>
...
public final class DataProviderEvent
{
   ...
   *          provider.
   */
  public DataProviderEvent(Message reason, Set<Type> types)
  {
    Validator.ensureNotNull(reason);
    Validator.ensureNotNull(types);
    Validator.ensureTrue(!types.isEmpty());
   ...
   *
   * @return The unmodifiable set containing the types of event that
   *         have occurred in the data provider.
   */
