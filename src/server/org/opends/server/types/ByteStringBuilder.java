...
public final class ByteStringBuilder implements ByteSequence
{
  ...
  private final class SubSequence implements ByteSequence
  {
    ...
    {
      return channel.write(ByteBuffer.wrap(buffer, subOffset,
          subLength));
    }
  ...
  /**
   * Appends the provided {@code ReadableByteChannel} to this byte
   * string builder.
   *
   * @param channel
   *          The {@code ReadableByteChannel} to be appended to this
   *          byte string builder.
   * @param length
   *          The maximum number of bytes to be appended from
   *          {@code channel}.
   * @return The number of bytes read, possibly zero, or {@code -1} if
      ...
      throws IndexOutOfBoundsException, IOException
  {
    ...
    ensureAdditionalCapacity(length);
    int bytesRead = channel.read(ByteBuffer.wrap(buffer, this.length,
        length));
    if (bytesRead > 0)
    {
      this.length += bytesRead;
  ...
  /**
   * Returns the current capacity of this byte string builder. The
   * capacity may increase as more data is appended.
   *
  ...
  /**
   * Sets the length of this byte string builder to zero, and resets
   * the capacity to the specified size.
   * <p>
   * <b>NOTE:</b> if this method is called, then
   * {@code ByteSequenceReader.rewind()} must also be called on any
   * associated byte sequence readers in order for them to remain
   * valid.
   *
   * @param capacity
  ...
  public ByteStringBuilder ensureAdditionalCapacity(int size)
  {
    ...
    {
      byte[] newbuffer = new byte[Math.max(buffer.length << 1,
          newCount)];
