package org.opends.client.api;

import org.opends.common.api.response.SearchResultDone;

/**
 * Created by IntelliJ IDEA.
 * User: boli
 * Date: Jul 7, 2009
 * Time: 3:34:25 PM
 * To change this template use File | Settings | File Templates.
 */
public class SearchRequestException  extends RequestException
{
  private SearchResultDone response;

  public SearchRequestException(Throwable cause) {
    super(cause);
  }

  public SearchRequestException(SearchResultDone response)
  {
    super(response.getDiagnosticMessage());
    this.response = response;
  }

  public SearchResultDone getResponse() {
    return response;
  }
}
