 ...
 * Directory Server.
 */
public class LDAPSearch extends ConsoleApplication
{
  ...
  private class LDAPSearchResultHandler implements SearchResultHandler
  {
    ...
    public void handleEntry(SearchResultEntry entry) {
      ...
      if(control != null && control instanceof EntryChangeNotificationControl)
      {
        EntryChangeNotificationControl dc =
            (EntryChangeNotificationControl)control;
        println(INFO_LDAPSEARCH_PSEARCH_CHANGE_TYPE.get(
            dc.getChangeType().toString()));
        String previousDN = dc.getPreviousDN();
        if (previousDN != null)
        {
          println(INFO_LDAPSEARCH_PSEARCH_PREVIOUS_DN.get(
              previousDN));
        }
      }
      control = entry.getControl(OID_ACCOUNT_USABLE_CONTROL);
      if(control != null && control instanceof AccountUsabilityControl.Response)
      {
        AccountUsabilityControl.Response dc =
            (AccountUsabilityControl.Response)control;
            ...
            if (dc.getRemainingGraceLogins() > 0)
            {
              println(
                  INFO_LDAPSEARCH_ACCTUSABLE_REMAINING_GRACE
                      .get(dc.getRemainingGraceLogins()));
            }
          }
          if (dc.isLocked())
          {
     ...
     * asynchronous search operation.
     *
     * @param reference The search result reference.
     */
     ...
     *
     * @param error The error result exception indicating why the asynchronous
  ...
  {
    int retCode = mainSearch(args, false, System.in, System.out, System.err);
  ...
  private int run(String[] args, boolean returnMatchingEntries)
  {
    ...
    ArgumentParser argParser = new ArgumentParser(LDAPSearch.class.getName(),
        toolDescription, false, true, 0, 0, "[filter] [attributes ...]");
    ...
    try
    {
      ...
      BooleanArgument showUsage = new BooleanArgument("showUsage", OPTION_SHORT_HELP,
          OPTION_LONG_HELP,
    ...
    SearchRequest search = Requests.newSearchRequest(baseDN.getValue(),
        scope, filters.get(0),
    ...
    try
    {
      ...
      {
       println(ERR_DESCRIPTION_INVALID_VERSION.get(
    ...
    {
       println(ERR_DESCRIPTION_INVALID_VERSION.get(
                String.valueOf(version.getValue())));
      return ResultCode.CLIENT_SIDE_PARAM_ERROR.intValue();
    }
    ...
    if (pSearchInfo.isPresent())
    {
      ...
      ArrayList<PersistentSearchChangeType> ct =
      ...
      PersistentSearchControl psearchControl =
           new PersistentSearchControl(changesOnly, returnECs,
               ct.toArray(new PersistentSearchChangeType[ct.size()]));
    ...
    if (vlvDescriptor.isPresent())
    {
      ...
      if (numTokens == 3)
      {
        try
        {
          ...
          search.addControl(
              new VLVControl.Request(beforeCount, afterCount, assertionValue));
        }
        catch (Exception e)
        {
      ...
      else if (numTokens == 4)
      {
        try
        {
          ...
          search.addControl(
              new VLVControl.Request(beforeCount, afterCount, offset,
                  contentCount));
        }
        catch (Exception e)
        {
    ...
    if (simplePageSize.isPresent())
    {
      ...
      try
      {
        ...
        search.addControl(
            new PagedResultsControl(pageSize, ByteString.empty()));
      }
      catch (ArgumentException ae)
      {
        Message message = ERR_ERROR_PARSING_ARGS.get(ae.getMessage());
    ...
    */
    ...
    try
    {
      ...
      SearchResultHandler resultHandler = new LDAPSearchResultHandler();
      while(true)
      {
        SearchResultFuture future = connection.search(search, resultHandler);
        SearchResult result = future.get();

        totalEntriesReturned += future.getNumSearchResultEntries();

        Control control = result.getControl(
            ServerSideSortControl.OID_SERVER_SIDE_SORT_RESPONSE_CONTROL);
        if(control != null && control instanceof ServerSideSortControl.Response)
        {
          ServerSideSortControl.Response dc =
              (ServerSideSortControl.Response)control;
          if (dc.getSortResult() != SortResult.SUCCESS)
          {
            Message msg =
                WARN_LDAPSEARCH_SORT_ERROR.get(dc.getSortResult().toString());
            println(msg);
          }
        }
        control = result.getControl(VLVControl.OID_VLV_RESPONSE_CONTROL);
        if(control != null && control instanceof VLVControl.Response)
        {
          ...
          else
          {
            Message msg = WARN_LDAPSEARCH_VLV_ERROR.get(
                dc.getVLVResult().toString());
            println(msg);
          }
        }

        control =
            result.getControl(PagedResultsControl.OID_PAGED_RESULTS_CONTROL);
        if(control != null && control instanceof PagedResultsControl)
        {
          ...
          if(pagedControl.getCookie().length() > 0)
          {
            ...
            search.removeControl(PagedResultsControl.OID_PAGED_RESULTS_CONTROL);
      ...
      {
        Message message =
            INFO_LDAPSEARCH_MATCHING_ENTRY_COUNT.get(totalEntriesReturned);
        println(message);
        println();
      }
    }
